...
def making_cmap_dict(soil_temp_min):
    ...
    if soil_temp_min >= 5:
        cmap_return = cmr.get_sub_cmap(color_to_use, 0.45, 0.85)
...
# 4 arrays of lat and lon of each weather station, and the station id and the soil temperature at that station
# df['StnID'].values means taking all data at the column names 'StnID' (and so on)
id = df['StnID'].values
lat = df['Lat'].values
lon = df['Long'].values
soil_tem = df['SoilTemp(20cm, Celsius)'].values.astype(float)
...
tem_at_extreme_east = soil_tem[lon_max_index]
tem_at_extreme_west = soil_tem[lon_min_index]
tem_at_extreme_north = soil_tem[lat_max_index]
...
soil_tem = np.append(soil_tem, tem_at_extreme_east)
soil_tem = np.append(soil_tem, tem_at_extreme_west)
soil_tem = np.append(soil_tem, tem_at_extreme_north)
...
xp, yp, soil_tem = remove_nan_observations(xp, yp, soil_tem)
...
alt_x, alt_y, data = interpolate_to_grid(xp, yp, soil_tem, minimum_neighbors=2, search_radius=240000, interp_type='barnes',
                                         hres=1000)
...
# Show the soil_temperature (in text) at each weather station on the plot

soil_tem = np.round(soil_tem, 1)  # Round the temperature data to 1 decimal place

# Loop through the temperature array and show it on the plot using the corresponding lat and lon
for i in range(len(soil_tem) - 2):
    if (i != lon_min_index):
        plt.annotate(str(soil_tem[i]), (xp[i], yp[i]), size=7)
    else:
        plt.annotate(str(soil_tem[i]), (xp[i] - 1, yp[i]), size=7)
...
# then we will choose what color map to use base on the current minimum soil temp
min_soil_tem = np.min(soil_tem)           # The minimum soil tem
cmap_to_use = making_cmap_dict(min_soil_tem)
...
cb.set_label('Celsius (\N{DEGREE SIGN}C)', size='x-large')
...
# The name of the plot is in the form: soil_temp_plot_Hour-Minute_Year-Month-Day.png, where Hour-Minute_Year-Month-Day is the
