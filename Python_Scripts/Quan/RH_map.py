...
4. Name of the color map to use
'''
#1.
...
def making_cmap_dict(min_RH):
    return_cmap = cmr.get_sub_cmap(color_to_use, 0.5, 1)
    if min_RH >= 50:
        return_cmap = cmr.get_sub_cmap(color_to_use, 0.5, 1)
    else:
        return_cmap = cmr.get_sub_cmap(color_to_use, 0, 0.5)
...
# 4 arrays of lat and lon of each weather station, and the station id and the RH at that station
...
RH_at_extreme_east = RH[lon_max_index]
RH_at_extreme_west = RH[lon_min_index]
RH_at_extreme_north = RH[lat_max_index]
...
RH = np.append(RH, RH_at_extreme_east)
RH = np.append(RH, RH_at_extreme_west)
RH = np.append(RH, RH_at_extreme_north)
...
xp, yp, RH = remove_nan_observations(xp, yp, RH)
...
alt_x, alt_y, data = interpolate_to_grid(xp, yp, RH, minimum_neighbors=2, search_radius=240000, interp_type='barnes',
                                         hres=1000)
...
# Show the RH (in text) at each weather station on the plot

RH = np.round(RH, 1)  # Round the RH data to 1 decimal place

# Loop through the RH array and show it on the plot using the corresponding lat and lon
for i in range(len(RH) - 2):
    if (i != lon_min_index):
        plt.annotate(str(RH[i]), (xp[i], yp[i]), size=7)
    else:
        plt.annotate(str(RH[i]), (xp[i] - 1, yp[i]), size=7)
...
cmap_to_use = making_cmap_dict(np.min(RH))
...
# The name of the plot is in the form: RH_plot_Hour-Minute_Year-Month-Day.png, where Hour-Minute_Year-Month-Day is the
