...
def making_cmap_dict(soil_mois_min):
...
# 4 arrays of lat and lon of each weather station, and the station id and the soil moisture at that station
# df['StnID'].values means taking all data at the column names 'StnID' (and so on)
id = df['StnID'].values
lat = df['Lat'].values
lon = df['Long'].values
soil_mois = df['SoilMoist(20cm, %)'].values.astype(float)
...
mois_at_extreme_east = soil_mois[lon_max_index]
mois_at_extreme_west = soil_mois[lon_min_index]
mois_at_extreme_north = soil_mois[lat_max_index]
...
soil_mois = np.append(soil_mois, mois_at_extreme_east)
soil_mois = np.append(soil_mois, mois_at_extreme_west)
soil_mois = np.append(soil_mois, mois_at_extreme_north)
...
xp, yp, soil_mois = remove_nan_observations(xp, yp, soil_mois)
...
alt_x, alt_y, data = interpolate_to_grid(xp, yp, soil_mois, minimum_neighbors=2, search_radius=240000,
...
# Show the soil_mois (in text) at each weather station on the plot

soil_mois = np.round(soil_mois, 1)  # Round the temperature data to 1 decimal place

# Loop through the temperature array and show it on the plot using the corresponding lat and lon
for i in range(len(soil_mois) - 2):
    if (i != lon_min_index):
        plt.annotate(str(soil_mois[i]), (xp[i], yp[i]), size=7)
    else:
        plt.annotate(str(soil_mois[i]), (xp[i] - 1, yp[i]), size=7)
...
# then we will choose what color map to use base on the current minimum soil moisture
min_soil_mois = np.min(soil_mois)           # The minimum soil moisture
cmap_to_use = making_cmap_dict(min_soil_mois)
...
cb.set_label('Percentage (%)', size='x-large')
...
# The name of the plot is in the form: soil_moisture_plot_Hour-Minute_Year-Month-Day.png, where Hour-Minute_Year-Month-Day is the
