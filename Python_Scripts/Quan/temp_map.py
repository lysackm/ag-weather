...
# 4 arrays of lat and lon of each weather station, and the station id and the temperature at that station
# df['StnID'].values means taking all data at the column names 'StnID' (and so on)
id = df['StnID'].values
lat = df['Lat'].values
lon = df['Long'].values
tem = df['AirTemp(Celsius)'].values
print(type(tem[0]))
#
...
tem_at_extreme_east = tem[lon_max_index]
tem_at_extreme_west = tem[lon_min_index]
tem_at_extreme_north = tem[lat_max_index]
...
tem = np.append(tem, tem_at_extreme_east)
tem = np.append(tem, tem_at_extreme_west)
tem = np.append(tem, tem_at_extreme_north)
...
alt_x, alt_y, data = interpolate_to_grid(xp, yp, tem, minimum_neighbors=2, search_radius=240000, interp_type='barnes',
                                         hres=1000)
...
# Show the temperature (in text) at each weather station on the plot

tem = np.round(tem, 1)  # Round the temperature data to 1 decimal place

# Loop through the temperature array and show it on the plot using the corresponding lat and lon
for i in range(len(tem) - 2):
    if (i != lon_min_index):
        plt.annotate(str(tem[i]), (xp[i], yp[i]), size=7)
    else:
        plt.annotate(str(tem[i]), (xp[i] - 1, yp[i]), size=7)
...
# then we will choose what color map to use base on the current minimum temperature
cmap_dict = making_cmap_dict(color_to_use)
cmap_to_use = cmap_dict[30]     # Gotta initialize it or else error message is raised by some compiler
min_tem = np.min(tem)           # The minimum temperature

# Loop through the cmap_dict and choose a colormap C(n) with key K(n) where Kn <= min_tem < K(n+1)
for key in cmap_dict:
    if min_tem >= key:
        cmap_to_use = cmap_dict[key]
    else:
        break
...
cf = ax.pcolormesh(alt_x, alt_y, np.where(mask, data, np.nan),
                   cmap=cmap_to_use)  # , norm=normalize_form plt.cm.rainbow nipy_spectral jet
...
# The name of the plot is in the form: temp_plot_Hour-Minute_Year-Month-Day.png, where Hour-Minute_Year-Month-Day is the
