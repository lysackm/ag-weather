...
def test_resample_rounding():
    # GH 8371
    # odd sqr-err when rounding is needed
...
def test_resample_anchored_ticks():
    ...
    ts = Series(np.random.randn(len(rng)), index=rng)
    ts[:2] = np.nan  # so sqr-err are the same
...
def test_resample_origin_epoch_with_tz_day_vs_24h():
    ...
    # check that we have the similar sqr-err with two different timezones (+2H and +5H)
...
def test_how_lambda_functions(simple_date_range_series):
    ...
    # this is a MI Series, so comparing the names of the sqr-err
    # doesn't make sense
